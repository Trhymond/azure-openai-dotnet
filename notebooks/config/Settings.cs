...
public static class Settings
{
    ...
    {
        var (useAzureOpenAI, model, gptModel, azureEndpoint, apiKey, orgId) = ReadSettings(_useAzureOpenAI, configFile);
        ...
        WriteSettings(configFile, useAzureOpenAI, model, gptModel, azureEndpoint, apiKey, orgId);
    ...
    public static (bool useAzureOpenAI, string model, string gptModel, string azureEndpoint, string apiKey, string orgId)
        LoadFromFile(string configFile = DefaultConfigFile)
    {
        ...
        try
        {
            ...
            string gptModel = config[GptModelKey];
            string azureEndpoint = config[EndpointKey];
            string apiKey = config[SecretKey];
            string orgId = config[OrgKey];
            if (orgId == "none") { orgId = ""; }

            return (useAzureOpenAI, model, gptModel, azureEndpoint, apiKey, orgId);
        }
        catch (Exception e)
        {
    ...
    private static (bool useAzureOpenAI, string model, string gptModel, string azureEndpoint, string apiKey, string orgId)
        ReadSettings(bool _useAzureOpenAI, string configFile)
    {
        ...
        try
        {
            if (File.Exists(configFile))
            {
                (useAzureOpenAI, model, gptModel, azureEndpoint, apiKey, orgId) = LoadFromFile(configFile);
            }
        }
        catch (Exception e)
        {
    ...
    private static void WriteSettings(
        string configFile, bool useAzureOpenAI, string model, string gptModel, string azureEndpoint, string apiKey, string orgId)
    {
        try
        {
